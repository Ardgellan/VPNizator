...
class Mailer:
    ...
    async def _set_unblocked_users(self, server_domain: str | None = None) -> None:
        """
        Получаем список пользователей, не заблокировавших бота.
        Если указан server_domain — фильтруем по нему.
        """
        if server_domain:
            self._unblocked_user_ids = await self._db_manager.get_unblocked_users_ids_by_domain(server_domain)
        else:
            self._unblocked_user_ids = await self._db_manager.get_unblocked_users_ids()

    async def run_mailing_post(self, server_domain: str | None = None) -> None:
        await self._set_unblocked_users(server_domain=server_domain)
        ...
        if not self._unblocked_user_ids:
            for admin in self._ADMINS:
                await self._message.bot.send_message(
                    chat_id=admin,
                    text="Не найдено пользователей для рассылки, все пользователи заблокировали бота или нет подходящих пользователей.",
                )
            return
